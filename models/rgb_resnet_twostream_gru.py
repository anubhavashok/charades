...
class TwoStreamNetworkLSTM(nn.Module):
    def __init__(self):
        ...
        print('Model: RGB/RESNET/GRU')
        USE_FLOW=False
        model = models.resnet18(pretrained=True)
        model.fc = nn.Linear(512, FEATURE_SIZE)
        self.RGBStream = model
        self.hidden_size = HIDDEN_SIZE
        self.lstm = torch.nn.GRU(FEATURE_SIZE, self.hidden_size, 2, bidirectional=True, dropout=0.5)
        self.rgbdropout = nn.Dropout(0.5)
        self.pool = nn.AvgPool1d(4)

    def reset_hidden(self, batch=1):
        self.h = Variable(torch.zeros(2*2, batch, self.hidden_size)).cuda()
